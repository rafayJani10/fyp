...
class _TeamBTabState extends State<TeamBTab> {
  ...
  Future deleteTeam(deleted_user_id) async {
    var collection = FirebaseFirestore.instance.collection('events');
    collection.doc(widget.eventId).update({
      'teamB': FieldValue.arrayRemove([deleted_user_id]),
    }).then((value) {
      showAlertDialog(
          context, "Successfull", "you deleted from this toyrnament");
      TeamB_joinUser = [];
      getTeamJoinUserList();
    });
  }
